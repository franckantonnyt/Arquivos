...
<div>
    <v-toolbar flat color="white">
        <v-toolbar-title>Parceiros</v-toolbar-title>
            ...
            <v-card>
                ...
                <v-card-text>
                    <v-container grid-list-md>
                        <v-layout wrap>
                            <v-flex xs12>
                                <v-text-field v-model="editedItem.name" label="Parceiro"></v-text-field>
                            </v-flex>
                                <v-flex xs12>
                            <v-text-field v-model="editedItem.logo" label="Logo"></v-text-field>
                                </v-flex>
                            <v-flex xs12>
                                <v-text-field v-model="editedItem.url" label="Url Parceiro"></v-text-field>
                            </v-flex>
                        </v-layout>
                    </v-container>
                </v-card-text>
                ...
                <v-data-table :headers="headers" :items="items" item-key="id" >
                    <template v-slot:items="props">
                        <tr>
                            ...
                            <td class="text-xs">{{ props.item.url }}</td>
  ...
  export default {
    ...
    data () {
      return {
            ...
            expand: false,
            headers: [
                { text: 'Parceiro', value: 'name', sortable: false},
                { text: 'Logo', value: 'logo', sortable: false},
                { text: 'Url Parceiro', value: 'url', sortable: false },
                {sortable: false}
            ],
            items: [],
            editedIndex: -1,
